...
test("reaction round writes are unavailable in a Classic Speed room", async () => {
  await seedRoom(room({mode: "classic-speed", roundStatus: "scheduled"}));

  await assertFails(
    set(ref(rtdb("host"), "games/room-1/reactionRounds/round-2"), {
      roundId: "round-2",
      status: "scheduled",
      windowMs: 5000
    })
...
test("only host can create and transition a secure round", async () => {
  await assertFails(
    set(ref(rtdb("guest"), "games/room-1/reactionRounds/round-2"), {
      roundId: "round-2",
      status: "scheduled",
      windowMs: 5000
    })
  ...
  await assertFails(
    set(
      ref(rtdb("guest"), "games/room-1/reactionRounds/round-1/status"),
      "signal"
    )
  );
  await assertSucceeds(
    set(
      ref(rtdb("host"), "games/room-1/reactionRounds/round-1/status"),
      "signal"
    )
  );
  await assertSucceeds(
